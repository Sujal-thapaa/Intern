...
export function getStateAbbreviation(stateName: string): string {
  const entry = Object.entries(stateAbbreviationMap).find(
    ([_, fullName]) => fullName.toLowerCase() === stateName.toLowerCase()
  )
  return entry ? entry[0] : stateName.substring(0, 2).toUpperCase()
}
