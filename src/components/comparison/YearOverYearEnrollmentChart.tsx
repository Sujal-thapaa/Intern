...
import { useEnrollmentByYear, useAvailableYears } from '@/hooks/useEnrollmentByYear'
...
export function YearOverYearEnrollmentChart() {
  ...
  // Fetch enrollment data for both years
  const {
    data: year1Data = [],
    isLoading: isLoadingYear1,
  } = useEnrollmentByYear(year1)

  const {
    data: year2Data = [],
    isLoading: isLoadingYear2,
  } = useEnrollmentByYear(year2)

  // Merge data for chart
  const chartData = useMemo<YearOverYearData[]>(() => {
        ...
        {isLoading ? (
          ...
          <div className="h-[400px] flex items-center justify-center text-muted-foreground border-2 border-dashed rounded-lg">
            {year1 === null || year2 === null
              ...
              : 'No enrollment data available for the selected years'}
          </div>
        ) : (
                      ...
                      <div className="bg-background border rounded-lg p-3 shadow-lg">
                        ...
                        {payload.map((entry, index) => (
                          <p key={index} className="text-sm" style={{ color: entry.color }}>
                            ...
                            {entry.value?.toLocaleString() || 0} enrollments
                          </p>
                        ))}
                      </div>
                    )
