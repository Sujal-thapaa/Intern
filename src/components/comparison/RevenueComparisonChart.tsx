...
import {
  useMonthlyRevenueByYear,
  useRevenueAvailableYears,
} from '@/hooks/useRevenueByYear'
import { formatCurrency } from '@/utils/currencyParser'

export interface YearOverYearRevenueData {
  month: string
  year1: number
  year2: number
}
...
export function RevenueComparisonChart() {
  ...
  // Fetch revenue data for both years
  const {
    data: year1Data = [],
    isLoading: isLoadingYear1,
  } = useMonthlyRevenueByYear(year1)

  const {
    data: year2Data = [],
    isLoading: isLoadingYear2,
  } = useMonthlyRevenueByYear(year2)

  // Merge data for chart
  const chartData = useMemo<YearOverYearRevenueData[]>(() => {
    if (!year1Data.length || !year2Data.length) return []

    return year1Data.map((item, index) => ({
      month: item.month,
      year1: item.revenue,
      year2: year2Data[index]?.revenue || 0,
    }))
  }, [year1Data, year2Data])
      ...
      <CardContent className="space-y-4">
        ...
        <div className="grid gap-4 md:grid-cols-2">
          <div className="space-y-2">
            <label className="text-sm font-medium">Select Revenue Year 1</label>
            {isLoadingYears ? (
              ...
              >
                <SelectTrigger>
                  <SelectValue placeholder="Select Revenue Year 1" />
                </SelectTrigger>
                <SelectContent>
          ...
          <div className="space-y-2">
            <label className="text-sm font-medium">Select Revenue Year 2</label>
            {isLoadingYears ? (
              <Skeleton className="h-10 w-full" />
            ) : (
              <Select
              ...
              >
                <SelectTrigger>
                  <SelectValue placeholder="Select Revenue Year 2" />
                </SelectTrigger>
                <SelectContent>
                  {availableYears.map((year) => (
                    <SelectItem key={year} value={year.toString()}>
        ...
        {isLoading ? (
          ...
          <div className="h-[400px] flex items-center justify-center text-muted-foreground border-2 border-dashed rounded-lg">
            {year1 === null || year2 === null
              ...
              : 'No revenue data available for the selected years'}
          </div>
        ) : (
